...
class DeviceControllerSpec extends Specification{
  ...
  object TestDeviceController extends DeviceController{
  ...
  "DeviceController" should {
    ...
    "populate the analogue in for the device" in new WithApplication {
      ...
      val changedThermometer:Device = TestDeviceController.readAndPopulateAnalogueIn(thermometer)
    ...
    "populate the analogue out for the device" in new WithApplication {
      ...
      val changedHeater:Device = TestDeviceController.readAndPopulateAnalogueOut(heater)
    ...
    "populate the digital out for the device" in new WithApplication {
      ...
      val changedPump:Device = TestDeviceController.readAndPopulateDigitalOut(pump)
      ...
      val changedPump2:Device = TestDeviceController.readAndPopulateDigitalOut(pump)
    ...
    "populate the digital in for the device" in new WithApplication {
      ...
      val changedSwitch:Device = TestDeviceController.readAndPopulateDigitalIn(switch)
